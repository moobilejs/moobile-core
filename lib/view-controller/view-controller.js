...
var ViewController = moobile.ViewController = new Class({
	...
	initialize: function(options, name) {
		...
		this.loadView();
		if (this.view) {
			this.view.on('ready', this.bound('__onViewDidBecomeReady'));
			this.view.on('layout', this.bound('__onViewDidUpdateLayout'));
			this.viewDidLoad();
		}
	...
	presentModalViewController: function(viewController, viewTransition) {
		...
		viewTransition.on('start:once', this.bound('__onPresentTransitionStart'));
		viewTransition.on('complete:once', this.bound('__onPresentTransitionCompleted'));
	...
	dismissModalViewController: function() {
		...
		viewTransition.on('start:once', this.bound('__onDismissTransitionStart'));
		viewTransition.on('complete:once', this.bound('__onDismissTransitionCompleted'));
	...
	destroy: function() {
		...
		this.view.off('ready', this.bound('__onViewDidBecomeReady'));
		this.view.off('layout', this.bound('__onViewDidUpdateLayout'));
		this.view.destroy();
		this.view = null;
