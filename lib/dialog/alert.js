...
var Alert = moobile.Alert = new Class({
	...
	willBuild: function() {
		...
		this.addClass('alert');
		this.on('animationend', this.bound('_onAnimationEnd'));
	...
	destroy: function() {
		...
		this.off('animationend', this.bound('_onAnimationEnd'));
	...
	didAddChildComponent: function(component) {
		...
		if (component instanceof moobile.Button) {
			component.on('tap', this.bound('_onButtonTap'));
	...
	didRemoveChildComponent: function(component) {
		this.parent(component);
		if (component instanceof moobile.Button) {
			component.off('tap', this.bound('_onButtonTap'));
	...
	_onButtonTap: function(e, sender) {
		...
		if (index >= 0) {
			this.emit('dismiss', [sender, index]);
		}
