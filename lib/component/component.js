...
var Component = moobile.Component = new Class({
	...
	on: function(type, fn, internal) {
		...
		if (this.supportNativeEvent(name)) {
			...
			if (callbacks[name].length === 1) this.element.on(name, listeners[name]);
		}
	...
	/**
	 * @see    http://moobilejs.com/doc/latest/Component/Component#off
	 * @author Jean-Philippe Dery (jeanphilippe.dery@gmail.com)
	 * @edited 0.3.0
	 * @since  0.1.0
	 */
		...
		if (this.supportNativeEvent(type)) {
			...
			if (callbacks[type] && callbacks[type].contains(fn)) {
				...
				if (callbacks[type].length === 0) {
					this.element.off(type, listeners[type]);
	...
	addChildComponent: function(component, where, context) {
		...
		context = context
			? document.id(context) || this.element.getElement(context)
			: this.element
	...
	__willAddChildComponent: function(component) {
		...
		this.emit('willaddchildcomponent', component);
	},
	...
	__didAddChildComponent: function(component) {
		...
		this.emit('didaddchildcomponent', component);
	},
	...
	__willRemoveChildComponent: function(component) {
		...
		this.emit('willremovechildcomponent', component)
	},
	...
	__didRemoveChildComponent: function(component) {
		...
		this.emit('didremovechildcomponent', component)
	},
	...
	__parentComponentWillChange: function(parent) {
		...
		this.emit('parentcomponentwillchange', parent)
	},
	...
	__parentComponentDidChange: function(parent) {
		...
		this.emit('parentcomponentdidchange', parent);
	},
	...
	__windowWillChange: function(window) {
		...
		this.emit('windowwillchange', window);
	},
	...
	__windowDidChange: function(window) {
		...
		this.emit('windowdidchange', window);
	},
	...
	__willChangeReadyState: function(ready) {
		...
		this.emit('willchangereadystate', ready)
	},
	...
	__didChangeReadyState: function(ready) {
		...
		this.emit('didchangereadystate', ready);
	},
