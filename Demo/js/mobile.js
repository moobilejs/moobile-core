...
Event.extend({
	...
	READY: Browser.Platform.phonegap ? 'deviceready' : 'domready',

	CLICK: Browser.Platform.desktop ? 'clickin' : 'touchin',

	PINCH: 'pinch',
	SWIPE: 'swipe',

	MOUSE_DOWN: Browser.Platform.desktop ? 'mousedown' : 'touchstart',
	MOUSE_MOVE: Browser.Platform.desktop ? 'mousemove' : 'touchmove',
	MOUSE_UP: Browser.Platform.desktop ? 'mouseup'   : 'touchend',
...
Moobile.Request.ViewController = new Class({
	...
	gotViewController: function(response) {
		...
		if (element) {
			...
			var v = this.createInstanceFrom(element, 'data-view', this.defaultView, element);
			var c = this.createInstanceFrom(element, 'data-controller', this.defaultController, v);
			var t = this.createInstanceFrom(element, 'data-transition', this.defaultTransition);
